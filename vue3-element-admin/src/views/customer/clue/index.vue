<template>
  <div class="app-container">
    <el-card class="search-card" shadow="never">
      ...
      <el-row class="clue-row" align="middle">
        <el-col :span="24">
          ...
          <el-link
            ...
            :key="item.value"
            :underline="false"
            :type="queryParams.type === item.value ? 'primary' : 'default'"
            class="clue-link"
        ...
        <el-col :span="24">
          <div class="clue-status-flex">
            ...
            <el-checkbox-group v-model="queryParams.Status" @change="handleQuery">
              <el-checkbox v-for="item in statusOptions" :key="item.value" :label="item.value">
                {{ item.label }}
              </el-checkbox>
            </el-checkbox-group>
          </div>
      ...
      <el-row class="clue-row" align="middle">
        <el-col :span="24">
          ...
          <el-link
            ...
            :key="item.value"
            :underline="false"
            :type="queryParams.OrderBy === item.value ? 'primary' : 'default'"
            class="clue-link"
          ...
          >
            ...
            <span class="order-arrows">
              <el-icon
                :class="[
                  'arrow-icon',
                  queryParams.OrderBy === item.value && queryParams.OrderDesc ? 'active' : '',
                ]"
              >
                <ArrowUp />
              </el-icon>
      ...
      <el-row class="clue-row clue-bottom" align="middle" justify="space-between">
        <el-col :span="12">
          ...
          <el-button type="warning" @click="handleResetQuery()">重置</el-button>
        </el-col>
        <el-col :span="12" class="clue-bottom-right">
          <el-input
            ...
            clearable
            style="width: 320px"
            class="mr8 clue-large-input clue-search-input"
          ...
          <el-dropdown>
            <el-button class="clue-large-btn">
              操作
              <el-icon>
                <ArrowDown />
              </el-icon>
      ...
      <div class="clue-detail-new-container">
        ...
        <div class="drawer-info-row">
          ...
          <div class="info-item">
            <span>创建时间</span>
            {{
              currentClue?.creationTime
                ? moment(currentClue.creationTime).format("YYYY-MM-DD HH:mm")
                : "--"
            }}
          </div>
          <!-- 创建人 -->
        ...
        <el-tabs v-model="activeTab" class="clue-detail-tabs" style="position: relative">
          ...
          <el-tab-pane label="联系记录" name="contact">
            <div class="contact-records">
              <!-- 添加联系记录按钮 -->
              <el-button type="primary" icon="Edit" @click="AddCommunication()">
                添加联系记录
              </el-button>
              <!-- 联系记录列表 -->
    ...
    <el-dialog v-model="addcommunicationdialogVisible" title="添加联系记录" width="900">
      ...
      <el-form
        ref="communicationruleFormRef"
        style="max-width: 600px"
        :model="communicationruleForm"
      ...
      >
        ...
        <el-form-item>
          <el-input
            v-model="communicationruleForm.content"
            type="textarea"
            :rows="12"
            placeholder=""
        ...
        <el-form-item>
          <el-select
            v-model="communicationruleForm.expectedDateId"
            placeholder="沟通类型"
            style="width: 140px"
          >
            <el-option
              v-for="item in communicationtypeList"
              :label="item.communicationTypeName"
              :value="item.id"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-if="communicationruleForm.isServe">
          <el-select
            v-model="communicationruleForm.customReplyId"
            placeholder="自定义回复"
            style="width: 140px"
          ...
          >
            <template v-if="customReplyList.length">
              <el-option
                v-for="item in customReplyList"
                :key="item.id"
                :label="item.customReplyName"
                :value="item.id"
              />
            </template>
            <template v-else>
        ...
        <el-form-item>
          <el-date-picker
            v-model="communicationruleForm.nextContactTime"
            placeholder="下次联系时间"
            style="width: 140px"
          />
        </el-form-item>
        ...
        <el-form-item>
          <el-checkbox v-model="communicationruleForm.isServe" style="margin-right: 8px">
            保存为跟进模板
          </el-checkbox>
        </el-form-item>
    ...
    <el-dialog v-model="uploadDialogVisible" title="上传附件" width="600">
      <el-upload
        ...
        action="/api/app/common/upload-file"
        :multiple="true"
        :auto-upload="false"
      ...
      >
        ...
        <el-button type="success" style="margin-left: 12px" @click="submitUpload">
          开始上传
        </el-button>
        <template #file="{ file }">
          <span>{{ file.name }}</span>
          <span v-if="uploadStatusMap[file.uid] === 'uploading'">
            <el-progress
              :percentage="uploadProgressMap[file.uid] || 0"
              style="width: 100px; display: inline-block; margin-left: 12px"
            />
          </span>
          <span
            v-else-if="uploadStatusMap[file.uid] === 'success'"
            style="color: #67c23a; margin-left: 12px"
          >
            上传成功
          </span>
          <span v-else>
            <el-button type="danger" size="small" @click.stop="handleRemoveFile(file)">
              删除
            </el-button>
          </span>
        </template>
        ...
        <tbody>
          <tr>
            <td>
              ...
              >
                共{{ queryParams.pageCount }}页 共{{ queryParams.totalCount }}条 当前第{{
                  queryParams.PageIndex
                }}/{{ queryParams.pageCount }}页
              </el-pagination>
            </td>
            ...
            <td>
              <el-pagination
                ...
                @current-change="handleCurrentChange"
              >
                <el-button :disabled="queryParams.PageIndex == 1" @click="handleCurrentChange(1)">
                  首页
                </el-button>
              </el-pagination>
            </td>
            ...
            <td>
              ...
              >
                <el-button
                  :disabled="queryParams.PageIndex == queryParams.pageCount"
                  @click="handleCurrentChange(queryParams.pageCount)"
                >
                  尾页
                </el-button>
              </el-pagination>
      ...
      <template #header>
        <div class="advanced-dialog-header">
          ...
          <div class="advanced-dialog-actions">
            <el-button type="primary" class="advanced-dialog-btn" @click="handleAdvancedSearch">
              搜索
            </el-button>
            <el-button
              type="primary"
              class="advanced-dialog-btn"
        ...
        <el-row :gutter="0">
          <el-col :span="24" style="margin-bottom: 16px">
            <div class="advanced-match-flex">
              ...
              <el-tooltip
                content="部分满足是指只要满足部分条件就查询出来，全部满足则查询满足全部条件的"
                placement="right"
              >
...
import {
  ...
  GetIndustry,
  AddClue,
} from "@/api/CustomerProcess/Clue/clue.api";
import {
  AddContactCommunication,
  GetContactCommunication,
  GetCommunicationType,
  GetCustomReplyByType,
...
const communicationrules = reactive<FormRules<communicationRuleForm>>({
  content: [{ required: true, message: "请填写沟通记录", trigger: "blur" }],
  expectedDateId: [{ required: true, message: "请选择沟通类型", trigger: "blur" }],
});
...
const rules = reactive<FormRules<RuleForm>>({
  clueName: [{ required: true, message: "姓名是必填项", trigger: "blur" }],
  cluePhone: [
...
const getStatusType = (status: number | string) => {
  ...
  switch (s) {
    case 0:
      return "primary"; // 未跟进 蓝色
    case 1:
      return "warning"; // 跟进中 橙色
    case 2:
      return "success"; // 已转换 绿色
    default:
      return "info";
  }
};
...
const selectCustomReply = async (typeId: string | number) => {
  ...
  await GetCustomReplyByType(typeId).then((res) => {
...
const selectCommunicationType = async () => {
  await GetCommunicationType().then((res) => {
...
const fetchClueList = async () => {
  ...
  try {
    ...
    const res: any = await ShowClueList(params);
...
watch(
  [() => communicationruleForm.expectedDateId, () => communicationruleForm.isServe],
